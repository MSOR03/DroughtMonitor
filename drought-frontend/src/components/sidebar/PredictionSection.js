...
import { TrendingUp, Zap, BarChart3, Map as MapIcon, Grid3x3, Droplets, Building2, Map as MapPolygon } from 'lucide-react';
...
export default function PredictionSection({
  ...
  const isCuencas = (predictionState.spatialUnit || 'grid') !== 'grid';
  const zonalNoun = { cuencas: 'cuenca', municipios: 'municipio', perimetro: 'perímetro' }[predictionState.spatialUnit] || 'zona';
  const source = predictionState.source || 'CHIRPS';
  const sourceLabel = (PREDICTION_SOURCES.find((s) => s.value === source) || {}).label || source;
  const gridResLabel = source === 'CHIRPS' ? '0.05°' : '0.1°';
  ...
  }, [predictionOpen, source]);

  const subtitle = useMemo(() => {
    if (loadingIssuedAt) return 'Cargando fecha de emision...';
    const issuedAtLabel = formatIssuedAt(currentIssuedAt);
    const base = issuedAtLabel ? `Fecha de emision: ${issuedAtLabel}` : 'Ultima prediccion disponible';
    return `${sourceLabel} · ${base}`;
  }, [currentIssuedAt, loadingIssuedAt, sourceLabel]);
    ...
    >
      ...
      <div className="space-y-5 p-5 bg-gradient-to-br from-green-50/50 via-green-50/20 to-green-50/30 dark:from-[#1a1f2e] dark:via-[#141920] dark:to-green-950/10 rounded-2xl border border-green-200/50 dark:border-green-900/30 shadow-lg">
        <div className="space-y-6">
          ...
          {/* Step 1: Data Source */}
          <StepSection step={1} title="Fuente de datos" color="green" collapsible defaultOpen>
            <div className="grid grid-cols-3 gap-2">
              {PREDICTION_SOURCES.map((s) => (
                <RadioCard
                  ...
                  onChange={() => setPredictionState((prev) => ({ ...prev, source: s.value }))}
                  label={s.label}
                  badge={s.description}
                />
              ))}
          ...
          <StepSection step={2} title="Tipo de visualizacion" color="green" collapsible defaultOpen>
            <div className="grid grid-cols-2 gap-2">
              ...
              <RadioOption
                ...
                label="2D Mapa"
                description={isCuencas ? `Agregado por ${zonalNoun}` : `Celdas ${sourceLabel} (${gridResLabel})`}
                icon={MapIcon}
              />
            </div>
          </StepSection>

          {/* Step 3: Spatial Unit (Celdas / Cuencas) */}
          <StepSection step={3} title="Unidad espacial" color="green" collapsible defaultOpen={false}>
            <div className="space-y-2">
              ...
              <RadioOption
                name="predSpatialUnit"
                value="cuencas"
                checked={predictionState.spatialUnit === 'cuencas'}
              ...
              <RadioOption
                name="predSpatialUnit"
                value="municipios"
                checked={predictionState.spatialUnit === 'municipios'}
                onChange={() => setPredictionState((prev) => ({ ...prev, spatialUnit: 'municipios' }))}
                label="Municipios"
                icon={Building2}
              />
              <RadioOption
                name="predSpatialUnit"
                value="perimetro"
                checked={predictionState.spatialUnit === 'perimetro'}
                onChange={() => setPredictionState((prev) => ({ ...prev, spatialUnit: 'perimetro' }))}
          ...
          <StepSection step={4} title="Indice de sequia" color="green" collapsible defaultOpen>
            <Select
          ...
          {/* Step 5: Scale */}
          <StepSection step={5} title="Nivel de Agregación" color="green" collapsible defaultOpen>
          ...
          {is2D && (
            <StepSection step={6} title="Horizonte de prediccion" color="green" collapsible defaultOpen>
              <div className="space-y-2">
